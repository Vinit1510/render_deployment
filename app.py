...
# Direct HTML Form Crown Claiming Engine
# ==========================================
...
class TimeBucksBot:
    def __init__(self):
        ...
        self.crown_status = "Direct Form Claim Engine Active ⚡"
    ...
    def claim_crown_direct(self):
        try:
            claim_url = "https://timebucks.com/publishers/index.php?pg=earn&tab=hourly_crown"
            self.log("⚡ DIRECT CROWN CLAIM EXECUTION: Sending Form POST (action=claim_crown)...")
            
            payload = {
                "action": "claim_crown",
                "tb_csrf_token": self.csrf_token
            }
            res = self.session.post(claim_url, data=payload, timeout=15)
            self.extract_live_data(res.text)
            
            if "You claimed the crown!" in res.text or "claimed" in res.text.lower():
                self.log("🎉 SUCCESS! TIMEBUCKS CONFIRMED: You claimed the crown! 👑")
                self.crown_status = "Crown Claimed! 👑"
            elif "Time Remaining until you can claim" in res.text:
                cd_match = re.search(r'claim crown again:\s*(\d+\s*mins\s*\d+\s*secs)', res.text)
                if cd_match:
                    self.log(f"⏳ Cooldown Active: {cd_match.group(1)}")
                    self.crown_status = f"Cooldown: {cd_match.group(1)}"
                else:
                    self.log("⏳ Cooldown active (Waiting for next cycle).")
            else:
                self.log(f"Crown Claim Response Received (HTTP {res.status_code}).")
        except Exception as e:
            self.log(f"Crown Claim Error: {e}")
    ...
    def run_cycle(self):
        ...
        # 3. Direct Crown Claim Execution
        self.claim_crown_direct()

    def loop(self):
        self.log("🚀 Direct Form Claim Engine Started!")
...
<body>
    <h1>👑 TimeBucks 24/7 Cloud Autopilot <span class="live-tag">DIRECT FORM CLAIM</span></h1>
    <div class="card">
